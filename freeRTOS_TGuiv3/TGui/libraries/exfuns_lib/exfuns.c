...
u8 fcpymsg(u8*pname,u8 pct,u8 mode)
{
	switch(mode)
	{
		//[0]:更新文件名
		case 0X01: 
			printf("%s fd is update \n",pname);
			break;

		//[1]:更新百分比pct
		case 0X02:
		{
			printf("%s file copyed %d\% \n",pname,pct);
			if(pct == 100)
				return 1;
		}	
			break;
...
u8 exf_copy(u8(*fcpymsg)(u8*pname,u8 pct,u8 mode),u8 *psrc,u8 *pdst,u32 totsize,u32 cpdsize,u8 fwmode)
{
	...
	{   
		...
		if(res==0)//两个都打开成功了
		{
			...
			{
				totsize=fsrc->fsize;//  obj.objsize;
				lcpdsize=0;
				curpct=0;
...
#define MAX_PATHNAME_DEPTH	512+1	//最大目标文件路径+文件名深度
	...
	if(res==0)
	{ 
 		dstpathname = (u8 *)malloc(MAX_PATHNAME_DEPTH);
		srcpathname = (u8 *)malloc(MAX_PATHNAME_DEPTH);
