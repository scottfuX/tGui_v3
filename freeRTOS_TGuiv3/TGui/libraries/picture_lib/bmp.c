...
u8 bmp_decode_buf(u8** out_buf,u16* width,u16* height,const char * filename)
{
FIL* f_bmp;
    u32 br;
	...
	if(res==0)//打开成功.
	{ 
		f_read(f_bmp,databuf,readlen,(u32*)&br);	//读出readlen个字节  
		...
		picinfo.bufaddr = (*out_buf);												// |	
		picinfo.bufwidth = (*width);												// |		
		picinfo.bufheight = (*height);												//-|	
				...
				if(color_byte==3)   //24位颜色图
				{
					switch (rgb) 
					{
						case 0:				  
							...
							break ;	   
						case 1: 	 
							color+=((u32)bmpbuf[count]<<8) & 0XFF00;//G
							break;	  
						case 2 : 
				...
				else if(color_byte==2)  //16位颜色图
				{
					switch(rgb)
					{
						case 0 : 
							...
							{
								color=((u32)bmpbuf[count]&0X1F);	 	//R
								color+=(((u32)bmpbuf[count])&0XE0)<<1; //G
							}
							else		//RGB:5,6,5
							{
						...
						case 1 : 			  			 
							if(biCompression==BI_RGB)//RGB:5,5,5
							{
								color+=(u32)bmpbuf[count]<<9;  //R,G
							}
							else  		//RGB:5,6,5
							{
								color+=(u32)bmpbuf[count]<<8;	//R,G
							}  									 
							break ;	 
					}		     
				}
...
{				
	...
	if((x+width-1)>GUI_WIDTH)return PIC_WINDOW_ERR;		//区域错误
	if((y+height-1)>GUI_HIGH)return PIC_WINDOW_ERR;	//区域错误 
 	...
 	if((hbmp.bmiHeader.biWidth*2)%4)//水平像素(字节)不为4的倍数
	{
 	...
 	if(res==FR_OK)//创建成功
	{
		res=f_write(f_bmp,(u8*)&hbmp,bmpheadsize,(UINT *)&bw);//写入BMP首部  
		for(ty=y+height-1;hbmp.bmiHeader.biHeight;ty--)
		{
			...
			res=f_write(f_bmp,(u8*)databuf,bi4width,(UINT *) &bw);//写入数据
		}
		f_close(f_bmp);
	}	    
#if BMP_USE_MALLOC == 1	//使用malloc	
