...
u8 *jpg_out_buffer;     //整个jpeg decode后的buffer
...
{ 
   	...
   	for(i=0;i<height;i++)//y坐标
	{
		...
		for(j=0;j<width;j++)//x坐标
		{
			...
			{
				pencolor += JPEG_PIXELSIZE;
				continue;
			}
#if JPEG_PIXELSIZE == 2
			color_tmp += ((*pencolor) << 8);
			color_tmp += *(pencolor + 1);
#else
			color_tmp += ((*pencolor) << 16);
			color_tmp += ((*(pencolor+ 1)) << 8);
			color_tmp += *(pencolor + 2);
			color_tmp |= 0xff000000;
#endif
			pic_phy.draw_point(realx+picinfo.S_XOFF,realy+picinfo.S_YOFF,color_tmp);//显示图片						
			pencolor += JPEG_PIXELSIZE;
			color_tmp = 0;
		}
	}
...
//返回值:0,解码成功;其他,解码失败.
u8 jpg_decode(const char *filename,u8 fast)
{  
	u8 res=0;	//返回值 
...
//fast:使能小图片(图片尺寸小于等于液晶分辨率)快速解码,0,不使能;1,使能.
//返回值:0,解码成功;其他,解码失败.
u8 jpg_decode_buf(u8** out_buf,u16* width,u16* height,const char *filename)
{  
	u8 res=0;	//返回值 
	u8 scale;	//图像输出比例 0,1/2,1/4,1/8  
	UINT (*outfun)(JDEC*, void*, JRECT*);
