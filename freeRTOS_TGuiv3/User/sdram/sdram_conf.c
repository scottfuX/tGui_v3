...
uint8_t SDRAM_Test(void)
{
  /*写入数据计数器*/
	...
	SDRAM_INFO("examing SDRAM, use 8bit/16bit to write/read sdram...");
	...
	for (counter = 0; counter < IS42S16400J_SIZE; counter++)
	{
		...
		{
			SDRAM_ERROR("8bit data read/write error!");
			return 0;
		}
	}
	...
	for (counter = 0; counter < IS42S16400J_SIZE / 2; counter++)
	{
		...
		{
			SDRAM_ERROR("16bit data read/write error !");
	...
	SDRAM_INFO("SDRAM read/write text success!"); 
	/*检测正常，return 1 */
	return 1;
